...
export function formatOrganizationOptionLabel(org: Partial<Organization>): string {
  if (!org.name) return "";
  const locationParts = [org.region, org.district].filter(Boolean);
    ...
    <div ref={containerRef} className="relative w-full">
      ...
      >
        <span className="truncate text-left font-normal text-foreground">
          {selectedOrg ? (
            formatOrganizationOptionLabel(selectedOrg)
          ) : (
        ...
        <div className="absolute z-50 mt-1 max-h-80 w-full overflow-hidden rounded-md border border-border bg-popover text-popover-foreground shadow-md transition-all animate-in fade-in-80 zoom-in-95">
          ...
          <div className="flex items-center border-b border-border px-3 py-2 bg-muted/30">
            ...
            <Input
              ...
              placeholder={t("common.search", { defaultValue: "Qidiruv (nomi, tuman)..." })}
          ...
          <div className="max-h-60 overflow-y-auto p-1">
            ...
            {isPending && displayedOrgs.length === 0 && (
            ...
            {!isPending && displayedOrgs.length === 0 && (
              <div className="p-4 text-center text-sm text-muted-foreground">
                {t("assignmentsAssignmentWizard.noOrganizations", {
                  defaultValue: "Tashkilot topilmadi",
                })}
              </div>
            )}
            ...
            {displayedOrgs.map((o) => {
              ...
              const label = formatOrganizationOptionLabel(o);
              return (
                <div
                  key={o.id}
                  onClick={() => {
